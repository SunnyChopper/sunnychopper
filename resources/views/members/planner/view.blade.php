	...
	<div class="container pt-64 pb-64">
		<div class="row justify-content-center">
			<div class="col-12">
				...
				<hr />
				<form action="/members/planner/update" id="update_planner_form" class="mt-32" method="POST">
					{{ csrf_field() }}
					<input type="hidden" name="planner_id" value="{{ $planner->id }}">
					<div class="form-group row">
						<div class="col-lg-6 col-md-6 col-sm-12 col-12">
							<label>Write Your Big Picture Goals (morning):</label>
							<textarea class="form-control" form="update_planner_form" name="morning_goals" rows="5">@if(isset($planner->morning_goals))
@foreach(json_decode($planner->morning_goals) as $goal){{ $goal . "\n" }}@endforeach
								@endif
							</textarea>
						</div>
						...
						<div class="col-lg-6 col-md-6 col-sm-12 col-12">
							...
							<textarea class="form-control" form="update_planner_form" name="night_goals" rows="5">@if(isset($planner->night_goals))
@foreach(json_decode($planner->night_goals) as $goal){{ $goal . "\n" }}@endforeach
								@endif
							</textarea>
						</div>
					</div>
					...
					<div class="form-group row">
						<div class="col-lg-6 col-md-6 col-sm-12 col-12">
							<label>Targets:</label>
							<textarea class="form-control" form="update_planner_form" name="targets" rows="6">
@foreach(json_decode($planner->targets) as $target){{ $target . "\n" }}@endforeach
							</textarea>
						</div>

						<div class="col-lg-6 col-md-6 col-sm-12 col-12">
							<label>Successes:</label>
							<textarea class="form-control" form="update_planner_form" name="successes" rows="6">@if(isset($planner->successes))
@foreach(json_decode($planner->successes) as $success){{ $success . "\n" }}@endforeach
								@endif
							</textarea>
						</div>
					</div>
