			...
			@if(count($books) > 0)
				@foreach($books as $book)
					<div class="col-lg-3 col-md-3 col-sm-6 col-6 mt-8 mb-8">
						...
						<h4 class="text-center mt-16">{{ $book->book_title }}</h4>
						<hr />
						<p class="text-center mb-16">{{ $book->description }}</p>
						<a href="{{ $book->link }}" class="btn btn-primary center-button">View Summary</a>
					</div>
				@endforeach
			@else
